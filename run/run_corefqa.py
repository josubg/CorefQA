...
try:
    ...
    flags.DEFINE_string("eval_dir", '', "MISSING")
    ...
    flags.DEFINE_bool("do_eval", False,
                      "Whether to do evaluation: evaluation is done on a set of trained checkpoints, the model will"
                      " select the best one on the dev set, and report result on the test set")
    ...
    flags.DEFINE_integer("window_size", 384,
                         "The number of sliding window size. Each document is split into a set of subdocuments with"
                         "length set to window_size.")
    flags.DEFINE_integer("num_window", 5, "The max number of windows for one document. This is used for fitting a \
        document into fix shape for TF computation. If a document is longer than num_window*window_size, the exceeding \
        part will be abandoned. This only affects training and does not affect test, since the all \
    ...
    flags.DEFINE_integer("num_tpu_cores", 1,
                         "[Optional] Only used if `use_tpu` is True. Total number of TPU cores to use.")
...
def main(_):
    ...
    if FLAGS.do_train:
        estimator.train(input_fn=file_based_input_fn_builder(FLAGS.train_file, num_window=FLAGS.num_window,
                                                             ...
                                                             max_num_mention=FLAGS.max_num_mention, is_training=True,
                                                             drop_remainder=True),
    ...
    if FLAGS.do_eval:
        ...
        checkpoints_iterator = [os.path.join(FLAGS.eval_dir, "model.ckpt-{}".format(str(int(ckpt_idx)))) for ckpt_idx in
                                range(0, num_train_steps + 1, FLAGS.save_checkpoints_steps)]
        ...
        for checkpoint_path in checkpoints_iterator[1:]:
            ...
            for result in estimator.predict(file_based_input_fn_builder(FLAGS.dev_file, num_window=FLAGS.num_window,
                ...
                predicted_clusters, gold_clusters, mention_to_predicted, mention_to_gold = model.evaluate(
                    result["topk_span_starts"], result["topk_span_ends"], result["top_antecedent"],
            ...
            tf.logging.info(
                "***** [DEV EVAL] ***** : precision: {:.4f}, recall: {:.4f}, f1: {:.4f}".format(dev_prec, dev_rec,
                                                                                                dev_f1))
            if dev_f1 > best_dev_f1:
                ...
                for result in estimator.predict(file_based_input_fn_builder(FLAGS.test_file,
                    ...
                    predicted_clusters, gold_clusters, mention_to_predicted, mention_to_gold = model.evaluate(
                        result["topk_span_starts"], result["topk_span_ends"], result["top_antecedent"],
                        result["cluster_ids"], result["gold_starts"], result["gold_ends"])
                    test_coref_evaluator.update(predicted_clusters, gold_clusters, mention_to_predicted,
                                                mention_to_gold)
                ...
                tf.logging.info(
                    "***** [TEST EVAL] ***** : precision: {:.4f}, recall: {:.4f}, f1: {:.4f}".format(test_pre, test_rec,
                                                                                                     test_f1))
        ...
        tf.logging.info(
            "- @@@@@ BEST DEV F1 : {:.4f}, Precision : {:.4f}, Recall : {:.4f},".format(best_dev_f1, best_dev_prec,
                                                                                        best_dev_rec))
        tf.logging.info(
            "- @@@@@ TEST when DEV best F1 : {:.4f}, Precision : {:.4f}, Recall : {:.4f},".format(
                test_f1_when_dev_best, test_prec_when_dev_best, test_rec_when_dev_best))
    ...
    if FLAGS.do_predict:
        ...
        for result in estimator.predict(file_based_input_fn_builder(
                ...
                window_size=FLAGS.window_size,
                max_num_mention=FLAGS.max_num_mention,
                is_training=False, drop_remainder=False), steps=698,
                checkpoint_path=checkpoint_path, yield_single_examples=False):
            predicted_clusters, gold_clusters, mention_to_predicted, mention_to_gold = model.evaluate(
